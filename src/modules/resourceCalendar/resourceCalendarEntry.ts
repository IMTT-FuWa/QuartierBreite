...
export class ResourceCalendarEntry {
  ...
  public adaptTimeForFullCalendar() {
    ...
    start = ResourceCalendarEntry.addHours(start, 1);
    end = ResourceCalendarEntry.addHours(end, 1);
  ...
  public convertToBackendAppointment(): any {
    ...
    for (let resource of this.resources) {
      mappedResources.push({ GGUID: resource });
    }
    if (mappedResources.length > 0) {
      appointment["resources"] = mappedResources;
    }
  ...
  public static adaptTimeZone(entry: ResourceCalendarEntry) {
    //TODO fix this when nudedian implemented ISO time standard
    entry.start = ResourceCalendarEntry.formatDateAndTime(ResourceCalendarEntry.addHours(new Date(entry.start), 2));
    entry.end = ResourceCalendarEntry.formatDateAndTime(ResourceCalendarEntry.addHours(new Date(entry.end), 2));
  }
