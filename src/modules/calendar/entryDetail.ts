...
export class CalendarEntryDetail extends ModuleBase implements OnInit {
  ...
  ionViewDidEnter() {
    ...
    this.appointmentService.getAppointment(this.entryParams.id).subscribe(appointment => {
      this.entry = new CalendarEntry(appointment);
      if (this.entry.participants) {
        for (var participant of this.entry.participants) {
          if (participant.displayname == "unbenannt") {
            participant.displayname = this.entryParams.data.group;
          }
        }
      }
    });
  ...
  export() {
    this.calendarImportService.exportSoniaAppointmentToDevice(this.entry);
  }
